...
export function parseTestReportSummary(
  ...
  const stdoutXmlReport = parseCapturedXmlReportSummary(limitReportText(stdout), target.framework);
  if (stdoutXmlReport === INVALID_TEST_REPORT) {
    return undefined;
  }
  const stderrXmlReport = parseCapturedXmlReportSummary(limitReportText(stderr), target.framework);
  if (stderrXmlReport === INVALID_TEST_REPORT) {
    return undefined;
  }
  const xmlReport = stdoutXmlReport ?? stderrXmlReport;
...
function parseCapturedXmlReportSummary(
  text: string,
  framework: TestFramework,
): TestReportSummary | InvalidTestReport | undefined {
...
function parseCapturedTestngXmlReportSummary(
  ...
  if (!looksLikeWholeTestngXmlReport(trimmed)) {
    if (startsLikeTestngXmlReport(trimmed)) {
      return INVALID_TEST_REPORT;
    }
    return undefined;
  }

  const counts = parseTestngXmlCounts(trimmed);
  if (!counts) {
    return INVALID_TEST_REPORT;
  }
...
function parseCapturedNunitXmlReportSummary(
  text: string,
  framework: TestFramework,
): TestReportSummary | InvalidTestReport | undefined {
  const trimmed = text.trim();
  ...
  if (!looksLikeWholeNunitXmlReport(trimmed)) {
    if (startsLikeNunitXmlReport(trimmed)) {
      return INVALID_TEST_REPORT;
    }
    return undefined;
  }

  const counts = parseNunitXmlCounts(trimmed);
  if (!counts) {
    return INVALID_TEST_REPORT;
  }
...
function parseCapturedRobotXmlReportSummary(
  text: string,
  framework: TestFramework,
): TestReportSummary | InvalidTestReport | undefined {
  const trimmed = text.trim();
  ...
  if (!looksLikeWholeRobotXmlReport(trimmed)) {
    if (startsLikeRobotXmlReport(trimmed)) {
      return INVALID_TEST_REPORT;
    }
    return undefined;
  }

  const counts = parseRobotXmlCounts(trimmed);
  if (!counts) {
    return INVALID_TEST_REPORT;
  }
...
function parseDartReportSummary(text: string, framework: TestFramework): TestReportSummary | InvalidTestReport | undefined {
  let counts:
    | {
        total: number;
        passed: number;
  ...
  for (const rawLine of text.split(/\r?\n/)) {
    const line = rawLine.trim();
    const candidate = parseDartStatusLine(line);
    if (candidate) {
      counts = candidate;
      continue;
    }
    if (looksLikeDartStatusLine(line)) {
      return INVALID_TEST_REPORT;
    }
  }
  if (!counts) {
...
function parseRobotReportSummary(text: string, framework: TestFramework): TestReportSummary | InvalidTestReport | undefined {
  let counts:
    | {
        total: number;
        passed: number;
  ...
  for (const rawLine of text.split(/\r?\n/)) {
    const line = rawLine.trim();
    const candidate = parseRobotStatusLine(line);
    if (candidate) {
      counts = candidate;
      continue;
    }
    if (looksLikeRobotStatusLine(line)) {
      return INVALID_TEST_REPORT;
    }
  }
  if (!counts) {
...
function parseJasmineReportSummary(text: string, framework: TestFramework): TestReportSummary | InvalidTestReport | undefined {
  let counts:
    | {
        total: number;
        passed: number;
  ...
  for (const rawLine of text.split(/\r?\n/)) {
    const line = rawLine.trim();
    const candidate = parseJasmineStatusLine(line);
    if (candidate) {
      counts = candidate;
      continue;
    }
    if (looksLikeJasmineStatusLine(line)) {
      return INVALID_TEST_REPORT;
    }
  }
  if (!counts) {
...
function parseBehatReportSummary(text: string, framework: TestFramework): TestReportSummary | InvalidTestReport | undefined {
  let scenarioCounts:
    | {
        total: number;
        passed: number;
  ...
  let counts:
    ...
    | undefined;
  let malformedFooter = false;

  for (const rawLine of text.split(/\r?\n/)) {
    const line = rawLine.trim();
    const scenarioCandidate = parseBehatCounterLine(line, "scenarios");
    if (scenarioCandidate) {
      scenarioCounts = scenarioCandidate;
      stepCounts = undefined;
      continue;
    }
    if (looksLikeBehatCounterLine(line, "scenarios")) {
      malformedFooter = true;
      scenarioCounts = undefined;
      stepCounts = undefined;
      continue;
    }

    const stepCandidate = parseBehatCounterLine(line, "steps");
    if (stepCandidate) {
      stepCounts = stepCandidate;
      continue;
    }
    if (looksLikeBehatCounterLine(line, "steps")) {
      malformedFooter = true;
      stepCounts = undefined;
      continue;
    }

    const durationMs = parseBehatTimerLine(line);
    if (durationMs !== undefined) {
      if (!scenarioCounts || !stepCounts || malformedFooter) {
        return INVALID_TEST_REPORT;
      }
      counts = {
        ...scenarioCounts,
      ...
      scenarioCounts = undefined;
      stepCounts = undefined;
      malformedFooter = false;
      continue;
    }
    if (looksLikeBehatTimerLine(line)) {
      if (scenarioCounts || stepCounts || malformedFooter) {
        return INVALID_TEST_REPORT;
      }
      continue;
...
function parseGoJsonReportSummary(text: string, framework: TestFramework): TestReportSummary | undefined {
  const finalActions = new Map<string, "passed" | "failed" | "skipped">();
  let durationMs: number | undefined;
  for (const rawLine of text.split(/\r?\n/)) {
    const line = rawLine.trim();
    ...
    const packageName = typeof parsed.Package === "string" && parsed.Package.trim() ? parsed.Package : undefined;
    if (!packageName || (action !== "pass" && action !== "fail" && action !== "skip")) {
      continue;
    }
    const testName = typeof parsed.Test === "string" && parsed.Test.trim() ? parsed.Test : undefined;
    if (!testName) {
      const elapsed = parseGoJsonElapsedMs(parsed.Elapsed);
      if ((action === "pass" || action === "fail") && elapsed !== undefined) {
        durationMs = sumDefined(durationMs, elapsed);
      }
      continue;
    }
    const key = `${packageName}\u0000${testName}`;
    finalActions.set(key, action === "pass" ? "passed" : action === "fail" ? "failed" : "skipped");
  }
...
function parseGoJsonElapsedMs(value: unknown): number | undefined {
...
function parseZigReportSummary(text: string, framework: TestFramework): TestReportSummary | InvalidTestReport | undefined {
  let counts:
    | {
        total: number;
        passed: number;
  ...
  for (const rawLine of text.split(/\r?\n/)) {
    ...
    const candidate = parseZigStatusLine(line);
    if (candidate) {
      counts = candidate;
      sawSummary = true;
      continue;
    }
    if (looksLikeZigStatusLine(line)) {
      return INVALID_TEST_REPORT;
    }
    if (sawSummary && !isZigPostSummaryDiagnosticLine(line)) {
      return INVALID_TEST_REPORT;
    }
  }
  if (!counts) {
...
function parsePestReportSummary(text: string, framework: TestFramework): TestReportSummary | InvalidTestReport | undefined {
  let counts:
    | {
        total: number;
        passed: number;
  ...
  for (const rawLine of text.split(/\r?\n/)) {
    const line = rawLine.trim();
    const candidate = parsePestStatusLine(line);
    if (candidate) {
      counts = candidate;
      continue;
    }
    if (looksLikePestStatusLine(line)) {
      return INVALID_TEST_REPORT;
    }
  }
  if (!counts) {
...
function parseMesonReportSummary(text: string, framework: TestFramework): TestReportSummary | InvalidTestReport | undefined {
  const counts: Partial<Record<"ok" | "expectedFail" | "fail" | "unexpectedPass" | "skipped" | "ignored" | "timeout", number>> = {};
  const seenLabels = new Set<keyof typeof counts>();
  let sawMesonLine = false;

  for (const rawLine of text.split(/\r?\n/)) {
    const line = stripTerminalControls(rawLine).trim();
    const candidate = parseMesonSummaryLine(line);
    if (candidate) {
      sawMesonLine = true;
    ...
    if (looksLikeMesonSummaryLine(line)) {
      return INVALID_TEST_REPORT;
    }
  }
  ...
  if (counts.ok === undefined || counts.fail === undefined) {
    return INVALID_TEST_REPORT;
  }

  const passed = counts.ok + (counts.expectedFail ?? 0);
  const failed = counts.fail + (counts.unexpectedPass ?? 0) + (counts.timeout ?? 0);
  const skipped = (counts.skipped ?? 0) + (counts.ignored ?? 0);
  const report: TestReportSummary = {
    framework,
    source: "meson",
  };
  assignNumber(report, "total", passed + failed + skipped);
  assignNumber(report, "passed", passed);
  assignNumber(report, "failed", failed);
  assignNumber(report, "skipped", skipped);
  return hasReportCounts(report) ? report : undefined;
...
function parseUnityReportSummary(text: string, framework: TestFramework): TestReportSummary | InvalidTestReport | undefined {
  let pending:
    | {
        total: number;
        failed: number;
  ...
  for (const rawLine of text.split(/\r?\n/)) {
    const line = stripTerminalControls(rawLine).trim();
    const candidate = parseUnitySummaryLine(line);
    if (candidate) {
      if (pending) {
        return INVALID_TEST_REPORT;
      }
    ...
    if (looksLikeUnitySummaryLine(line)) {
      return INVALID_TEST_REPORT;
    }
    if (line === "OK" || line === "FAIL" || line === "FAILED") {
      if (!pending) {
        continue;
      }
      if ((line === "OK" && pending.failed > 0) || (line !== "OK" && pending.failed === 0)) {
        return INVALID_TEST_REPORT;
      }
      counts = {
        ...pending,
        passed: pending.total - pending.failed - pending.skipped,
      };
      pending = undefined;
      continue;
    }
    if (pending && line !== "" && !/^-+$/.test(line)) {
      return INVALID_TEST_REPORT;
    }
  }
...
function parseMesonSummaryLine(line: string):
  | {
      label: "ok" | "expectedFail" | "fail" | "unexpectedPass" | "skipped" | "ignored" | "timeout";
      count: number;
    }
  | undefined {
  const match = /^(Ok|Expected Fail|Fail|Unexpected Pass|Skipped|Ignored|Timeout):\s+(\d+)\s*$/.exec(line);
  if (!match) {
    return undefined;
  }
  ...
  const label = match[1] ?? "";
  if (label === "Ok") {
    return { label: "ok", count };
  }
  if (label === "Expected Fail") {
    return { label: "expectedFail", count };
  }
  if (label === "Fail") {
    return { label: "fail", count };
  }
  if (label === "Unexpected Pass") {
    return { label: "unexpectedPass", count };
  }
  if (label === "Skipped") {
    return { label: "skipped", count };
  }
  if (label === "Ignored") {
    return { label: "ignored", count };
  }
  return { label: "timeout", count };
}

function looksLikeMesonSummaryLine(line: string): boolean {
  return /^(?:Ok|Expected Fail|Fail|Unexpected Pass|Skipped|Ignored|Timeout):/.test(line);
}

function parseUnitySummaryLine(line: string): {
  total: number;
  failed: number;
  skipped: number;
} | undefined {
  const match = /^(\d+)\s+Tests\s+(\d+)\s+Failures\s+(\d+)\s+Ignored$/.exec(line);
  if (!match) {
    return undefined;
  }
  const total = Number.parseInt(match[1] ?? "", 10);
  ...
  if (![total, failed, skipped].every((value) => Number.isInteger(value) && value >= 0) || total <= 0) {
    return undefined;
  }
...
function parsePestStatusLine(line: string): {
  total: number;
  passed: number;
  failed: number;
  skipped: number;
  ...
  const match = /^Tests:\s+(.+)\s+\(\d+\s+assertions?\)$/.exec(line);
  if (!match) {
    return undefined;
  }
  const counts = {
  ...
  for (const rawPart of (match[1] ?? "").split(",")) {
    const partMatch = /^\s*(\d+)\s+(deprecated|warnings?|notices?|incomplete|todos?|skipped|failed|passed)\s*$/.exec(rawPart);
    if (!partMatch) {
      return undefined;
    }
    const value = Number.parseInt(partMatch[1] ?? "", 10);
    const label = partMatch[2] ?? "";
    const normalizedLabel = label.endsWith("s") ? label.slice(0, -1) : label;
    if (!Number.isInteger(value) || value < 0 || seenLabels.has(normalizedLabel)) {
      return undefined;
    }
    ...
    } else if (label === "incomplete" || label === "todo" || label === "todos") {
      counts.todo += value;
    } else {
      counts.passed += value;
    }
...
function looksLikePestStatusLine(line: string): boolean {
  return /^Tests:\s+.*\(\d+\s+assertions?\)/.test(line);
}
...
function parseDartStatusLine(line: string): {
  total: number;
  passed: number;
  failed: number;
  skipped: number;
  ...
  const match = /^(\d{1,2}:\d{2}(?::\d{2})?)\s+\+(\d+)(?:\s+-(\d+))?(?:\s+~(\d+))?:\s+(All tests passed!|Some tests failed\.|All tests skipped\.)$/.exec(line);
  if (!match) {
    return undefined;
  }
  const durationMs = parseClockDurationMs(match[1] ?? "");
  const passed = Number.parseInt(match[2] ?? "", 10);
  const failed = match[3] === undefined ? 0 : Number.parseInt(match[3], 10);
  const skipped = match[4] === undefined ? 0 : Number.parseInt(match[4], 10);
  const status = match[5] ?? "";
  if (
    durationMs === undefined ||
    ![passed, failed, skipped].every((value) => Number.isInteger(value) && value >= 0)
  ) {
    return undefined;
  }
  if (status === "All tests passed!" && (passed === 0 || failed !== 0)) {
    return undefined;
  }
  if (status === "Some tests failed." && failed === 0) {
    return undefined;
  }
  if (status === "All tests skipped." && (passed !== 0 || failed !== 0 || skipped === 0)) {
    return undefined;
  }
  return {
    total: passed + failed + skipped,
...
function looksLikeDartStatusLine(line: string): boolean {
  return /^\d{1,2}:\d{2}(?::\d{2})?\s+\+\d+(?:\s+-\d+)?(?:\s+~\d+)?:\s+(?:All tests passed!|Some tests failed\.|All tests skipped\.)/.test(line);
}
...
function parseRobotStatusLine(line: string): {
  total: number;
  passed: number;
  failed: number;
  skipped: number;
} | undefined {
  const match = /^(\d+)\s+tests?,\s+(\d+)\s+passed,\s+(\d+)\s+failed(?:,\s+(\d+)\s+skipped)?$/.exec(line);
  if (!match) {
    return undefined;
  }
  const total = Number.parseInt(match[1] ?? "", 10);
  ...
  const failed = Number.parseInt(match[3] ?? "", 10);
  const skipped = match[4] === undefined ? 0 : Number.parseInt(match[4], 10);
  const values = [total, passed, failed, skipped];
  if (!values.every((value) => Number.isInteger(value) && value >= 0) || total <= 0) {
    return undefined;
  }
  if (passed + failed + skipped !== total) {
...
function looksLikeRobotStatusLine(line: string): boolean {
  return /^\d+\s+tests?,\s+/.test(line);
}

function parseJasmineStatusLine(line: string): {
  total: number;
  passed: number;
  failed: number;
  skipped: number;
} | undefined {
  const match = /^(\d+)\s+specs?,\s+(\d+)\s+failures?(?:,\s+(\d+)\s+pending\s+specs?)?$/.exec(line);
  if (!match) {
    return undefined;
  }
  const total = Number.parseInt(match[1] ?? "", 10);
...
function looksLikeJasmineStatusLine(line: string): boolean {
  return /^\d+\s+specs?,\s+/.test(line);
}
...
function parseBehatCounterLine(line: string, kind: "scenarios" | "steps"): {
  ...
  const noun = kind === "scenarios" ? "scenarios?" : "steps?";
  const match = new RegExp(`^(\\d+)\\s+${noun}\\s+\\(([^)]+)\\)$`).exec(line);
  if (!match) {
    return undefined;
  }
  const total = Number.parseInt(match[1] ?? "", 10);
  ...
  const seenLabels = new Set<string>();
  for (const part of (match[2] ?? "").split(",")) {
    const partMatch = /^\s*(\d+)\s+(passed|failed|skipped|pending|undefined)\s*$/.exec(part);
    if (!partMatch) {
      return undefined;
    }
    const value = Number.parseInt(partMatch[1] ?? "", 10);
  ...
  if (counts.passed + counts.failed + counts.skipped + counts.todo !== total) {
    return undefined;
  }
...
function looksLikeBehatCounterLine(line: string, kind: "scenarios" | "steps"): boolean {
  const noun = kind === "scenarios" ? "scenarios?" : "steps?";
  return new RegExp(`^\\d+\\s+${noun}\\b`).test(line);
}

function parseBehatTimerLine(line: string): number | undefined {
  const match = /^(\d+)m(\d+(?:\.\d+)?)s\s+\(\d+(?:\.\d+)?(?:B|Kb|Mb|Gb|Tb)\)$/.exec(line);
  if (!match) {
    return undefined;
  }
  const minutes = Number.parseInt(match[1] ?? "", 10);
  const seconds = Number.parseFloat(match[2] ?? "");
  if (!Number.isInteger(minutes) || minutes < 0 || !Number.isFinite(seconds) || seconds < 0) {
    return undefined;
  }
  return Math.round((minutes * 60 + seconds) * 1000);
}

function looksLikeBehatTimerLine(line: string): boolean {
  return /^\d+m\d+(?:\.\d+)?s\s+\(/.test(line);
}
...
function parseZigStatusLine(line: string): {
  ...
  const allPassedMatch = /^All\s+(\d+)\s+tests passed\.$/.exec(line);
  if (allPassedMatch) {
    const total = Number.parseInt(allPassedMatch[1] ?? "", 10);
    if (!Number.isInteger(total) || total <= 0) {
      return undefined;
    }
    return {
  ...
  const countsMatch = /^(\d+)\s+passed([;,])\s+(\d+)\s+skipped\2\s+(\d+)\s+failed(\.)?$/.exec(line);
  if (!countsMatch) {
    return undefined;
  }
  const passed = Number.parseInt(countsMatch[1] ?? "", 10);
  const skipped = Number.parseInt(countsMatch[3] ?? "", 10);
  const failed = Number.parseInt(countsMatch[4] ?? "", 10);
  const separator = countsMatch[2] ?? "";
  const period = countsMatch[5];
  if (![passed, skipped, failed].every((value) => Number.isInteger(value) && value >= 0)) {
    return undefined;
  }
  if ((separator === ";" && period !== ".") || (separator === "," && period !== undefined)) {
    return undefined;
  }
...
function looksLikeZigStatusLine(line: string): boolean {
  return /^All\s+\d+\s+tests? passed\.?$/.test(line) || /^\d+\s+passed[;,]\s+\d+\s+skipped[;,]/.test(line);
}

function isZigPostSummaryDiagnosticLine(line: string): boolean {
  return /^\d+\s+errors were logged\.$/.test(line) ||
    /^\d+\s+tests leaked memory\.$/.test(line) ||
    /^\d+\s+fuzz tests found\.$/.test(line);
}
...
function parseCypressSummaryLine(line: string): {
  ...
  const durationMs = parseClockDurationMs(match[4] ?? "");
...
function looksLikeWholeTestngXmlReport(text: string): boolean {
  return matchWholeXmlRootName(stripXmlDeclaration(text).trim()) === "testng-results";
}

function startsLikeTestngXmlReport(text: string): boolean {
  return matchFirstXmlRootName(stripXmlDeclaration(text).trim()) === "testng-results";
}
...
function looksLikeWholeRobotXmlReport(text: string): boolean {
  return matchWholeXmlRootName(stripXmlDeclaration(text).trim()) === "robot";
}

function startsLikeRobotXmlReport(text: string): boolean {
...
function collectXmlStartTagAttributesAtPath(text: string, tagPath: string[]): string[] | undefined {
  const attributes: string[] = [];
  const normalizedPath = tagPath.map((part) => part.toLowerCase());
  const stack: string[] = [];
  let searchIndex = 0;
  ...
  while (searchIndex < text.length) {
    ...
    if (xmlTagPathMatches([...stack, name], normalizedPath)) {
      attributes.push(extractXmlStartTagAttributes(token.tag));
    }
    if (!isSelfClosingXmlTag(token.tag)) {
      stack.push(name);
    }
  }
...
function xmlTagPathMatches(actual: string[], expected: string[]): boolean {
...
function parseTestngXmlCounts(text: string):
  | {
      total: number;
      passed: number;
      failed: number;
  ...
  | undefined {
  const rootTags = collectXmlStartTagAttributes(text, "testng-results");
  const suiteTags = collectXmlStartTagAttributes(text, "suite");
  if (!rootTags || !suiteTags || rootTags.length !== 1) {
    return undefined;
  }

  const rootAttributes = rootTags[0];
  const total = matchXmlIntegerAttribute(rootAttributes, "total");
  const passed = matchXmlIntegerAttribute(rootAttributes, "passed");
  const failed = matchXmlIntegerAttribute(rootAttributes, "failed");
  const skipped = matchOptionalXmlIntegerAttribute(rootAttributes, "skipped", 0);
  const ignored = matchOptionalXmlIntegerAttribute(rootAttributes, "ignored", 0);
  if (
    total === undefined ||
    passed === undefined ||
    failed === undefined ||
  ...
  const skippedTotal = skipped + ignored;
  if (!Number.isSafeInteger(skippedTotal) || passed + failed + skippedTotal !== total) {
    return undefined;
  }

  const durationMs = sumTestngSuiteDurations(suiteTags);
  if (durationMs === null) {
    return undefined;
  }
  ...
  return {
    ...
    failed,
    skipped: skippedTotal,
    suites: suiteTags.length || undefined,
    durationMs,
  };
}

function sumTestngSuiteDurations(tags: string[]): number | undefined | null {
  let total = 0;
  let hasDuration = false;
  for (const tag of tags) {
    const duration = matchOptionalXmlDecimalAttribute(tag, "duration-ms");
    if (duration === null) {
      return null;
    }
    if (duration !== undefined) {
  ...
  return hasDuration ? Math.round(total * 1000) / 1000 : undefined;
}

function parseNunitXmlCounts(text: string):
  | {
      total: number;
      passed: number;
      failed: number;
...
function parseNunit3XmlCounts(text: string):
  ...
  const passed = matchXmlIntegerAttribute(rootAttributes, "passed");
  const failed = matchXmlIntegerAttribute(rootAttributes, "failed");
  const warnings = matchOptionalXmlIntegerAttribute(rootAttributes, "warnings", 0);
  const inconclusive = matchOptionalXmlIntegerAttribute(rootAttributes, "inconclusive", 0);
  const skipped = matchOptionalXmlIntegerAttribute(rootAttributes, "skipped", 0);
  const testcaseCount = matchOptionalXmlIntegerAttribute(rootAttributes, "testcasecount");
  const seconds = matchOptionalXmlDecimalAttribute(rootAttributes, "duration");
  if (
    total === undefined ||
    passed === undefined ||
    failed === undefined ||
  ...
  const passedTotal = passed + warnings;
  const skippedTotal = skipped + inconclusive;
  if (
    ![passedTotal, skippedTotal].every(Number.isSafeInteger) ||
    passedTotal + failed + skippedTotal !== total ||
    (testcaseCount !== undefined && testcaseCount !== total)
  ) {
    return undefined;
  }
...
function parseRobotXmlCounts(text: string):
  | {
      total: number;
      passed: number;
      failed: number;
  ...
  | undefined {
  const statisticsTags = collectXmlStartTagAttributesAtPath(text, ["robot", "statistics"]);
  const totalTags = collectXmlStartTagAttributesAtPath(text, ["robot", "statistics", "total"]);
  const statTags = collectXmlStartTagAttributesAtPath(text, ["robot", "statistics", "total", "stat"]);
  if (
    !statisticsTags ||
    !totalTags ||
    !statTags ||
    statisticsTags.length !== 1 ||
    totalTags.length !== 1 ||
    statTags.length !== 1
  ) {
    return undefined;
  ...
  return parseRobotXmlTotalStatAttributes(statTags[0]);
}

function parseRobotXmlTotalStatAttributes(attributes: string):
  | {
      total: number;
      passed: number;
      failed: number;
  ...
  | undefined {
  const passed = matchXmlIntegerAttribute(attributes, "pass");
  const failed = matchXmlIntegerAttribute(attributes, "fail");
  const skipped = matchOptionalXmlIntegerAttribute(attributes, "skip", 0);
  if (passed === undefined || failed === undefined || skipped === null) {
    return undefined;
  }
