...
- Local coding tools: file/search/shell/diff/patch model tools, native `run_tests`, `orx tests` with discovery-only JSON target listings, structured test-run JSON, and structured report summaries, code maps, symbols, refs, imports, lexical call graphs with structured JSON output, optional local ast-grep previews, and optional local tree-sitter parse/file-inventory previews.
