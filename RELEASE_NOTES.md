...
- Broader test report integrations for additional non-JSON/custom reporters or wrapper commands beyond the current direct Node, captured TAP/Mocha/pytest/Cargo/Go/RSpec/Python-unittest/JUnit-text/PHPUnit/dotnet/CTest-style summaries, declared/config JSON output, existing JSON reporter, and exact default Jest/Vitest/Playwright runner paths.
