...
- Explicit operator integrations: TypeScript/Pyright/ESLint/gopls/clangd diagnostics, Semgrep scanner profile, MCP preset/profile management, guarded MCP calls/model grants, plugin scaffold/validate/install/review, trusted plugin bins/hooks, skills/prompts/rules, and policy-gated delegation.
...
- Additional scanner/diagnostics profiles beyond the current Semgrep and TypeScript/Pyright/ESLint/gopls/clangd set only after deterministic no-network/no-auth command shapes are proven.
