...
orx code map
orx map src
orx web fetch https://example.com
orx web search openrouter models
orx web browse https://example.com
...
The release gate is deterministic and no-real-key/no-network by command selection. It clears operator `OPENROUTER_API_KEY` and `BRAVE_SEARCH_API_KEY` values, runs `git diff --check`, typecheck, full `npm test`, the global install verifier, npm package dry-run content assertions, and built CLI smokes for `doctor --json`, `guide`, `code calls`, `plugins review`, `plugins review --json`, `plugins validate --json`, and `mcp presets` against isolated temporary ORX state. The package dry-run check verifies required runtime files while excluding source, memory, scripts, local ORX state, lockfiles, and compiled test/spec artifacts from the published package. The nested global-install chat-launch smoke uses a non-secret placeholder key only to start chat and immediately run `/exit`; it does not submit prompts or call OpenRouter. The gate does not call remote MCP endpoints, plugin bins, or plugin hooks. See `RELEASE_NOTES.md` for the current v0.1 handoff summary and known optional post-v0.1 work.
...
orx code tree-sitter src/cli.ts --json
orx code outline src/cli.ts
...
orx code tree-sitter repo-files src
orx code tree-sitter repo-files src --json
...
Research profiles are read-only planning metadata. `orx research profiles [--json]`, `orx research profiles inspect <profile> [--json]`, `orx research profiles plan <profile> [--json]`, `orx web profiles ...`, and `/web profiles ...` list what is available today and what remains catalog-only without fetching URLs, calling Brave, launching browsers, writing state, or exposing model tools. `orx web fetch <url>` and `orx research fetch <url>` fetch a guarded URL directly and print source metadata without writing session state. `orx web search <query>` and `orx research search <query>` call Brave Web Search only when `BRAVE_SEARCH_API_KEY` is configured and render secondary provider-snippet evidence without fetching primary pages. `orx web browse <url>` and `orx research browse <url>` create browser evidence snapshots when Playwright is available. Chat slash forms keep their existing evidence ledger behavior through `/web fetch`, `/web search`, `/web browse`, `/sources`, `/cite`, and `/bibliography`. `research-crawl`, `research-scholar`, `research-docs`, `research-rag`, and `research-memory` remain catalog-only until their source, storage, citation, and model-visible retrieval contracts are designed.
...
orx mcp presets
orx mcp presets --json
orx mcp presets search github
orx mcp presets search "code search" --json
...
orx mcp presets inspect gitlab-ci-write
orx mcp plan gitlab-ci-write
orx mcp add-preset gitlab-ci-write
...
The `gitlab-ci-write` built-in preset targets the same GitLab.com MCP endpoint and declares GitLab's `manage_pipeline` tool as destructive. It is auth-required, high-risk, write-capable=yes, and remains disabled until the operator enables/trusts the profile, supplies compatible auth, and grants the destructive tool explicitly.
...
/mcp presets
/mcp presets --json
/mcp presets search github
/mcp presets search "code search" --json
/mcp presets inspect deepwiki
/mcp presets inspect deepwiki --json
...
/mcp presets inspect gitlab-ci-write
/mcp plan gitlab-ci-write
...
/tests [list [--json]|status [--json]|run [target-id] [--json] [-- args...]]
/map [path] [--json]
