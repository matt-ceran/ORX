...
- optional tree-sitter AST parse, outline, and single-file call-edge previews through an installed local `tree-sitter` binary and local grammars, with shell disabled, cleaned env, cwd-confined file guards, bounded/redacted output, source-range name extraction for outlines/calls when possible, and no mutation flags
