...
- Delegation execution policy storage is private local operator state at `~/.orx/delegation/policy.json` or `ORX_DELEGATION_POLICY_PATH`. It writes `0600` files under private default directories, refuses symlink paths, stores only numeric future execution limits and fixed inert policy strings, and never stores API keys, credentials, prompts, or delegate outputs.
- `orx delegates policy`, `orx delegate policy`, `/delegates policy`, and `/delegate policy` render the disabled future execution policy. `policy set` can tune max task cost, timeout, result bytes, max concurrent delegates, credential forwarding, result persistence, and result merge mode, but credential forwarding remains `none`, result persistence remains `none`, result merge remains `manual_summary`, and execution remains disabled.
- The internal `delegate_task` runtime contract now exists behind explicit runtime wiring only. Normal `ask` and `chat` do not expose its schema. Explicit dispatch validates safe bounded args against the disabled policy, returns `execution_disabled` or `adapter_unavailable`, records hash-only audit metadata at `~/.orx/audit/delegation.jsonl` or `ORX_DELEGATION_AUDIT_PATH`, and never stores raw prompts, raw context, expected output text, API keys, or delegate output.
- `/status` and `orx status` show `delegation_audit_path`, `delegate_task_runtime: policy_enforced_disabled`, `delegate_task_model_exposure: unavailable`, and `delegate_task_adapter: unavailable`.
...
- Interactive `/status` shows orchestration controller, disabled execution state, delegate count, saved delegation team count, delegation policy path/limits, and `delegate_task=unavailable`.
...
- Delegation readiness output must continue to show execution disabled, no network calls, no subprocesses, no normal model-visible `delegate_task`, and blockers for the OpenRouter adapter plus live enforcement before adapters are implemented.
