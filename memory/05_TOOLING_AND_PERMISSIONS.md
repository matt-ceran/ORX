...
- optional tree-sitter AST parse, outline, single-file import-source previews, and single-file call-edge previews through an installed local `tree-sitter` binary and local grammars, with shell disabled, cleaned env, cwd-confined file guards, bounded/redacted output, source-range name extraction for outlines/imports/calls when possible, and no mutation flags
