...
- optional tree-sitter AST parse, outline, single-file import-source previews, single-file identifier refs, bounded repo identifier refs, and single-file call-edge previews through an installed local `tree-sitter` binary and local grammars, with shell disabled, cleaned env, cwd-confined file/repo path guards, bounded/redacted output, source-range name extraction for outlines/imports/refs/calls when possible, and no mutation flags
...
The code-map adapter is local-only, no-key, and not model-autonomous. It reads bounded local file metadata/content, redacts secret-like rendered paths, symbols, references, and call graph fields, skips symlinks, and reports omissions/truncation instead of following unbounded trees. The ast-grep and tree-sitter adapters are also explicit/operator-only: ORX never installs ast-grep, tree-sitter, or parser grammars, never calls network, never exposes these surfaces as model tools, and never mutates files; missing local binaries return setup guidance and a nonzero CLI exit while lexical code intelligence remains available. Tree-sitter import, refs, repo-refs, and call extraction are AST help only: imports map single-file import-like AST ranges to module specifiers, refs map exact identifier-like queries to AST identifier nodes in one file, repo-refs scan bounded local source files for exact AST identifier matches, and calls map single-file call-expression ranges to names plus nearest named enclosing definitions; these modes do not claim semantic resolution.
