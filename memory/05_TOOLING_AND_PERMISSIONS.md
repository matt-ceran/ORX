...
- inferred Node/Vitest/Jest/Playwright/AVA/unknown framework metadata and simple reporter hints for package-script targets
- direct Node test file fallback when no `test` package script exists
- compact parsed report counts from direct Node JUnit, private temporary Jest/Vitest/Playwright JSON report files for package scripts that already declare append-safe JSON reporters, exact final default framework runner invocations, or exact no-prefix `npx --no-install` framework runner invocations, changed cwd-confined JSON report output files already declared by package scripts, per-run args, or fixed local framework config files for exact final direct/no-install npx runner scripts, whole-object Jest/Vitest/Playwright/Mocha/RSpec JSON already emitted to stdout/stderr, common Node/Vitest/Jest/Playwright summary output, AVA summary output for AVA-inferred targets, captured TAP reporter output, captured Mocha JSON report output, captured RSpec JSON report output, captured Mocha-style summary output, captured pytest-style summary output, captured Cargo/Rust summary output, captured cargo-nextest summary output, captured Cucumber scenario summary output, captured Behat full summary footer output, captured Behave summary footer output, captured testthat bracket summary output, captured GoogleTest summary output, captured Catch2 summary output, captured Dart compact final output, captured Deno-style summary output, captured ExUnit summary output, captured Gradle-style summary output, captured JUnit Platform console summary output, captured ScalaTest summary output, captured TestNG summary output, captured NUnit summary output, captured Robot Framework summary output, captured Jasmine summary output, captured Go test verbose or `go test -json` output, captured Cypress run-summary output, captured RSpec summary output, captured Minitest summary output, captured Karma-style summary output, captured Bun-style summary output, captured Tasty console summary output, captured Zig native summary output, captured Python unittest summary output, captured JUnit/Surefire-style text summary output, captured whole JUnit XML report output, captured TestNG XML report output, captured NUnit XML report output, captured xUnit XML report output, captured TRX XML report output, captured Robot Framework XML report output, captured CTest XML report output, captured TeamCity service-message output, captured Pest summary output, captured PHPUnit-style text summary output, captured `.NET test`/xUnit summary output, captured CTest summary output, captured Meson summary output, captured Unity summary footer output, captured LLVM lit summary output, captured Bazel aggregate summary output, and captured XCTest summary output
- explicit operator commands `orx tests list|status|run`, `/tests list|status|run`, and `/test`
- model-visible native tool `run_tests`

The test adapter is available to the operator and model loop. `orx tests --json`, `orx tests list --json`, `orx tests status --json`, and matching slash forms emit ORX-owned structured discovery metadata only: no test execution, binary probing, report-file reads, installs, network calls, run behavior changes, or model-tool exposure changes. Runs use the shared process runner with shell disabled, bounded output, timeout coverage, and sanitized extra arguments. Report parsing reads only ORX-owned private temp reports for direct Node, append-safe existing JSON reporter package scripts, exact final no-reporter `jest`/`vitest`/`playwright test` package-script invocations, exact no-prefix `npx --no-install <jest|vitest|playwright|playwright-core|@playwright/test>` package-script invocations, changed cwd-confined JSON report files that the script or per-run args already declared, changed cwd-confined JSON report files declared by fixed local framework config files for exact final direct/no-install npx runner scripts, and already-captured sanitized stdout/stderr, then stores numeric summary fields including TAP, Mocha JSON, RSpec JSON, Mocha-style, pytest-style, Cargo/Rust, cargo-nextest, AVA for AVA-inferred targets, Cucumber scenarios, Behat full summary footers, Behave summary footers, testthat brackets, GoogleTest summaries, Catch2 summaries, Dart compact final lines, Deno-style, ExUnit, Gradle-style, JUnit Platform console, ScalaTest, TestNG, NUnit, Robot Framework, Jasmine, Go test verbose, `go test -json`, Cypress run summaries, RSpec, Minitest, Karma-style, Bun-style, Tasty console, Zig native, Python unittest, JUnit/Surefire-style text, whole JUnit XML reports, whole TestNG XML reports, whole NUnit XML reports, whole xUnit XML reports, whole TRX XML reports, whole Robot Framework XML reports, whole CTest XML reports, TeamCity service messages, Pest, PHPUnit-style text, `.NET test`/xUnit, CTest, Meson, Unity, LLVM lit, Bazel aggregate, or XCTest counts when those outputs are present. ORX does not execute framework config files, does not create/mutate/delete declared output files, and unchanged/stale declared reports are ignored. Plain npx, path-like npx or runner tokens, env/cross-env-prefixed npx, separator forms, duplicate or non-lowercase `--no-install`, other npx options, wrapper commands, additional non-JSON/custom reporters, config-only outputs for non-direct/no-install npx scripts, and unsafe multi-step package-script shapes stay on stdout/stderr fallback. `/status` and `orx status` show discovered target counts, framework counts, and the default target.
...
- optional tree-sitter AST parse, outline, bounded repo outline previews, single-file import-source previews, single-file identifier refs, bounded repo identifier refs, single-file call-edge previews, bounded repo call-expression previews, bounded repo import-source previews, and bounded local-relative plus safe root tsconfig/jsconfig path/baseUrl repo dependency previews through an installed local `tree-sitter` binary and local grammars, with shell disabled, cleaned env, cwd-confined file/repo path guards, bounded/redacted output, source-range name extraction for outlines/imports/refs/calls/deps when possible, and no mutation flags
...
The code-map adapter is local-only, no-key, and not model-autonomous. It reads bounded local file metadata/content, redacts secret-like rendered paths, symbols, references, and call graph fields, skips symlinks, and reports omissions/truncation instead of following unbounded trees. The ast-grep and tree-sitter adapters are also explicit/operator-only: ORX never installs ast-grep, tree-sitter, or parser grammars, never calls network, never exposes these surfaces as model tools, and never mutates files; missing local binaries return setup guidance and a nonzero CLI exit while lexical code intelligence remains available. Tree-sitter outline, repo-outline, import, refs, repo-refs, calls, repo-calls, repo-imports, and repo-deps extraction are AST help only: outline maps single-file definition-like AST ranges, repo-outline scans bounded local source files for path-aware AST definition-like previews, imports map single-file import-like AST ranges to module specifiers, refs map exact identifier-like queries to AST identifier nodes in one file, repo-refs scan bounded local source files for exact AST identifier matches, calls map single-file call-expression ranges to names plus nearest named enclosing definitions, repo-calls scan bounded local source files for path-aware AST call-expression previews, repo-imports scan bounded local source files for path-aware AST import-source previews, and repo-deps resolve local relative AST import sources plus safe root `tsconfig.json` or `jsconfig.json` `paths`/`baseUrl` aliases against the bounded scanned source set while counting external and unresolved-local imports; these modes do not claim semantic symbol/reference/call, package-manager, or full dependency resolution.

Implemented under `src/security/`:

- scanner profile catalog for Semgrep, Snyk, Socket, OSV-Scanner, CodeQL, and Trivy
- explicit operator commands `orx scanners list|status [--json]`, `orx scanners inspect|show <profile> [--json]`, `orx scanners plan|setup-plan <profile> [--json]`, `orx scanners run <semgrep|trivy|codeql|osv-scanner> <path-or-database> [--config <local-config-path>] [--query <local-query-or-suite>] [--json]`, `orx scan <semgrep|trivy|codeql|osv-scanner> ...`, `/scanners ...`, and `/scan ...`
...
- runnable CodeQL database-analysis adapter only when a local `codeql` binary is already installed, an existing local database directory is provided, and a local query, suite, or query directory under cwd is provided through `--query`
- runnable OSV-Scanner adapter only when a local `osv-scanner` binary is already installed and already-cached offline vulnerability data is available; it runs source/lockfile vulnerability scanning only and rejects `--config`/`--query`

The scanner adapter is local-only, no-key, and not model-autonomous. ORX never installs Semgrep, Trivy, CodeQL, OSV-Scanner, Snyk, or Socket, never uses Semgrep registry/URL configs, never loads Trivy, CodeQL, or OSV config files, never forwards ORX/OpenRouter/Brave/API token-like env values, and never exposes scanners as model tools. List/status, inspect/show, and plan/setup-plan readiness surfaces can render ORX-owned structured JSON without process execution or binary probing; plan/setup-plan surfaces also avoid process spawn, network access, and state writes while showing guarded next commands or catalog-only blockers. Semgrep runs use shell-disabled process execution, `--metrics off`, cwd-confined target/config path guards with symlink realpath checks, dash-prefixed operand rejection, secret/control-character argument rejection, and bounded/redacted stdout/stderr. Trivy runs use shell-disabled process execution with `trivy fs --scanners secret --format json --offline-scan --skip-db-update --skip-java-db-update --skip-check-update --skip-version-check --disable-telemetry --no-progress <path>`, cwd-confined regular-file-or-directory target guards with symlink realpath checks, secret/control-character argument rejection, and bounded/redacted stdout/stderr/JSON passthrough; vulnerability, misconfiguration, license, image, and registry scans are not enabled. CodeQL runs use shell-disabled process execution with `codeql database analyze --format=sarifv2.1.0 --output=<orx-temp-sarif> --no-download --no-sarif-add-file-contents --no-sarif-add-snippets --sarif-include-query-help=never --no-print-diagnostics-summary --no-print-metrics-summary --threads=0 -- <database> <query>`, require a cwd-confined existing database directory plus cwd-confined local `.ql`, `.qls`, or query directory, reject `--config`, URLs, pack names, dash-prefixed operands, symlink escapes, secrets, and control characters, and do not create databases or resolve remote packs. OSV-Scanner runs use shell-disabled process execution with `osv-scanner scan source --recursive --format json --offline --no-resolve <path>`, require a cwd-confined regular-file-or-directory target, reject `--config`, `--query`, URLs, dash-prefixed operands, symlink escapes, secrets, and control characters, do not pass `--download-offline-databases`, and do not enable online matching, image scanning, license scanning, or guided remediation. Snyk and Socket remain catalog/readiness-only until no-network/no-auth local command shapes are proven.
...
- runnable Ruff adapter only when an already-installed local, virtualenv, or PATH `ruff` binary is available
- runnable Mypy adapter only when an already-installed local, virtualenv, or PATH `mypy` binary is available
- runnable gopls adapter only when an already-installed local or PATH `gopls` binary is available
- runnable clangd adapter only when an already-installed local or PATH `clangd` binary is available

The diagnostics adapter is local-only, no-key, and not model-autonomous. ORX never installs TypeScript, Pyright, ESLint, Ruff, Mypy, gopls, clangd, Go toolchains, Python packages, C/C++ toolchains, Node packages, or language packages, never invokes package managers or launchers, never forwards ORX/OpenRouter/Brave/API token-like env values, and never exposes diagnostics as model tools. List/status and inspect/show readiness surfaces can render ORX-owned structured JSON without process execution or binary probing. TypeScript runs use shell-disabled process execution, `tsc --noEmit --pretty false --project <tsconfig>`, cwd-confined project-file guards with symlink realpath checks, URL/registry/package/dash/control/secret-like argument rejection, bounded/redacted stdout/stderr, parsed TypeScript diagnostic summaries, and optional ORX-owned JSON metadata. Pyright runs use the same process/env guards with `pyright --outputjson --project <project-file-or-directory>`, a cwd-confined file-or-directory project target defaulting to `.`, parsed `generalDiagnostics`, and optional ORX-owned JSON metadata. ESLint runs use the same process/env guards with `eslint --format json <file-or-directory>`, a cwd-confined file-or-directory target defaulting to `.`, parsed JSON `messages`, and optional ORX-owned JSON metadata. Ruff runs use the same process/env guards with `ruff check --output-format json --no-cache <file-or-directory>`, project-local `node_modules/.bin/ruff` or cwd virtualenv `ruff` before PATH, a cwd-confined file-or-directory target defaulting to `.`, parsed JSON diagnostics, and optional ORX-owned JSON metadata. Mypy runs use the same process/env guards with `mypy --no-color-output --no-error-summary --show-column-numbers --no-incremental --cache-dir <null-device> <file-or-directory>`, project-local `node_modules/.bin/mypy` or cwd virtualenv `mypy` before PATH, a cwd-confined file-or-directory target defaulting to `.`, parsed text diagnostics, no `.mypy_cache` writes by command selection, and optional ORX-owned JSON metadata. gopls runs require `--project <local-go-file>`, probe PATH with `gopls version`, use `gopls check <go-file>` with a cwd-confined regular `.go` file guard, parse text diagnostics, set `GOPROXY=off`, `GOSUMDB=off`, and `GOTOOLCHAIN=local` in the child env, and support optional ORX-owned JSON metadata. clangd runs require `--project <local-c-cpp-source-or-header-file>`, probe PATH with `clangd --version`, use `clangd --log=error --check=<file>` with a cwd-confined regular C/C++/Objective-C source/header file guard, parse `clangd --check` stderr diagnostics, and support optional ORX-owned JSON metadata. TypeScript Language Server, rust-analyzer, and SCIP TypeScript remain catalog/readiness-only until a no-network/no-auth local command shape is proven.
...
- `orx mcp presets search|find <query> [--json]` and matching slash commands search built-in provider preset metadata only. They reject missing, option-like, control-character, overlong, and secret-like queries, and must not install, enable, trust, grant, fetch, call, audit, expose model tools, write catalog state, execute processes, or contact network.
