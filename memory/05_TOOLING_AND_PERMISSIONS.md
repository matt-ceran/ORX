...
- Enabled plugins can declare MCP presets through cached `components.mcpServers` JSON. ORX namespaces these profiles as `plugin:<plugin-id>:<server-id>`, includes plugin manifest/component hashes in the MCP profile hash, and shows them through `/mcp list`, `/mcp inspect`, `/mcp tools`, `/mcp call`, `/mcp remote-tools`, `/mcp enable`, `/mcp discover`, and `/status`. `/mcp discover` may contact enabled, trusted, unchanged plugin `remote-http` endpoints only through the guarded DNS-vetted transport and only for a minimal initialize handshake. `/mcp remote-tools` may call `tools/list` through the same gates, but renders only bounded untrusted metadata and hashes. `/mcp call` and `orx mcp call` may execute declared plugin MCP tools only through explicit operator commands and the same policy/auth/audit gates. `/mcp allow-model-tool` / `orx mcp allow-model-tool` persist active model grants for read-only non-billable plugin MCP tools, and `/mcp model enable` or `orx ask --mcp-tools` can expose only those model-granted tools through `mcp_call`; billable/write/destructive model-loop plugin MCP tools remain inactive.
...
- Bins require trusted current hashes before execution, including when run through `/plugin:<plugin-id>:bin:<file>` aliases. Derived prompt aliases only activate untrusted prompt context. Custom manifest-defined executable plugin command schemas remain inactive even when a plugin is marked enabled. Plugin-declared MCP tools require separate MCP profile enablement/trust and explicit `/mcp call`, `orx mcp call`, or read-only model grants plus `/mcp model enable` / `orx ask --mcp-tools` before model-visible execution.
- `/plugins inspect`, `/skills`, `/prompts`, `/rules`, `/hooks`, `/bins`, `/plugin list`, and `/status` show this trust boundary explicitly; enabled hook/bin counts reflect trusted current hashes, while custom executable plugin command schemas remain inactive.
