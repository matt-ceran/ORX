...
`npm run overnight:init`, `npm run overnight`, and `npm run overnight:dashboard` are local operator automation around ORX work, not model-visible runtime tools. They store gitignored state/prompts/logs/events under `.orx/overnight/latest`, render a fixed-screen sanitized dashboard from local state, and run explicit implementor/verifier commands selected by the operator. The default role command is built ORX through `node dist/cli.js ask --max-tool-iterations 32 <prompt>`. The harness itself does not fetch remote MCP tools, run plugin bins/hooks, or expose new model tools; any network/model behavior comes only from the chosen role command. Optional `--commit`/`--push` happen only after local checks and final-line verifier `VERDICT: PASS`; commit mode scopes to paths newly changed during the verified slice, and `--require-clean` is available when a dirty-worktree preflight is desired.
...
- optional tree-sitter AST parse previews through an installed local `tree-sitter` binary and local grammars, with shell disabled, cleaned env, cwd-confined file guards, bounded/redacted output, and no mutation flags
- explicit operator commands `orx code map`, `orx map`, `orx code-map`, `orx code symbols`, `orx symbols`, `orx code refs`, `orx refs`, `orx code imports`, `orx imports`, `orx code calls`, `orx calls`, `orx call-graph`, `orx code ast-grep`, `orx ast-grep`, `orx code tree-sitter`, `orx tree-sitter`, `/map`, `/code map`, `/code symbols`, `/symbols`, `/code refs`, `/refs`, `/code imports`, `/imports`, `/code calls`, `/calls`, `/call-graph`, `/code ast-grep`, `/ast-grep`, `/code tree-sitter`, and `/tree-sitter`
