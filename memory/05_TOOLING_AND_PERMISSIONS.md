...
- explicit operator commands `orx scanners list|status [--json]`, `orx scanners inspect|show <profile> [--json]`, `orx scanners plan|setup-plan <profile> [--json]`, `orx scanners run <semgrep|trivy|codeql> <path-or-database> [--config <local-config-path>] [--query <local-query-or-suite>] [--json]`, `orx scan <semgrep|trivy|codeql> ...`, `/scanners ...`, and `/scan ...`
