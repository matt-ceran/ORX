...
- Enabled plugins can declare MCP presets through cached `components.mcpServers` JSON. ORX namespaces these profiles as `plugin:<plugin-id>:<server-id>`, includes plugin manifest/component hashes in the MCP profile hash, and shows them through `/mcp list`, `/mcp inspect`, `/mcp tools`, `/mcp enable`, `/mcp discover`, and `/status`. `/mcp discover` may contact enabled, trusted, unchanged plugin `remote-http` endpoints only through the guarded DNS-vetted discovery transport and only for a minimal initialize handshake; no plugin MCP tools are executable.
