...
- `orx orchestrator`, `orx delegate`, and `orx delegates` are noninteractive no-key/session-less parity commands. They render delegation status/readiness without an API key; live-session mutating forms validate arguments and refuse because there is no active chat session to mutate.
...
- `orx delegates policy`, `orx delegate policy`, `/delegates policy`, and `/delegate policy` render and update the policy. `policy set` can tune max task cost, timeout, result bytes, max concurrent delegates, and explicit `--execution enabled|disabled`; credential forwarding remains `none`, result persistence remains `none`, and result merge remains `manual_summary`.
- The internal `delegate_task` runtime contract now has a policy-gated OpenRouter adapter. Normal `ask` does not expose its schema. Interactive chat exposes it only when policy execution is enabled and the chat session has at least one delegate. Live dispatch validates safe bounded args, rejects provider-token/bearer/assignment-shaped secret-like live payloads before network, streams one OpenRouter delegate call, returns explicitly wrapped untrusted output with begin/end markers and structured `untrustedOutputPolicy`, records hash-only audit metadata at `~/.orx/audit/delegation.jsonl` or `ORX_DELEGATION_AUDIT_PATH`, and never stores raw prompts, raw context, expected output text, API keys, or raw delegate output in audit.
