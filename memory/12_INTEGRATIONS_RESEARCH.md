...
- Current implementation supports local plugin catalog editor commands, `orx plugins catalog add-local|add-git|remove` and `/plugins catalog add-local|add-git|remove`, as private local declaration management only. They do not install, enable, trust, grant, fetch, or execute plugin surfaces.
