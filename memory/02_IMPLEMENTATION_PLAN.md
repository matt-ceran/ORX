...
Status: complete as of 2026-06-26. Persistence foundation, `/resume`, and persistent-session-aware `/compact` are verified.
...
- Expose delegation through an ORX-owned `delegate_task` tool so ORX enforces credentials, permissions, budgets, and result merging. Current staged status: policy-gated OpenRouter delegate execution exists for interactive chat only when execution is explicitly enabled and a delegate is configured; `orx ask`, subprocess/external-agent adapters, automatic result merge, and stronger pre-spend budget strategy remain future work.
