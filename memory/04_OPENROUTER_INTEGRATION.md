...
- `src/mcp/` contains a disabled-by-default `openrouter` profile for `https://mcp.openrouter.ai/mcp`, persisted profile trust state, schema/profile hashing, audit scaffolding, gated remote HTTP discovery, guarded `tools/list`, reviewed user-catalog tool import, per-tool grants, and explicit operator `tools/call`.
...
- `/mcp remote-tools <profile>` and `orx mcp remote-tools <profile>` can call guarded `tools/list` for enabled/trusted/unchanged remote HTTP profiles and render bounded untrusted metadata plus schema hashes.
- `/mcp import-remote-tools <profile>` and `orx mcp import-remote-tools <profile>` can import reviewed remote tool names into local `user:` catalog profiles only as read-only/free declarations, then require profile retrust when the declaration hash changes.
...
- Interactive `/mcp model enable` adds the ORX-owned `mcp_call` native model tool for the current chat session only. `orx ask --mcp-tools` adds the same bridge for one noninteractive request. It is limited to read-only non-billable declared MCP tools with active model-tool grants and returns redacted bounded remote output as explicitly wrapped untrusted tool context. Direct REST continues to power `/models`, `/credits`, `/generation`, `orx models`, `orx credits`, `orx generation`, and normal OpenRouter inference.
