...
- Completed:
  ...
  - Add built-in MCP provider presets: `orx mcp presets`, `orx mcp add-preset <preset>`, `/mcp presets`, and `/mcp add-preset <preset>` install disabled local user catalog declarations for `context7`, `microsoft-learn`, and `github-readonly`.
