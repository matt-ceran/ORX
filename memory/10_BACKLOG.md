...
- Add documented provider preset packs/templates for local user MCP catalogs and/or plugins now that local catalog add/remove commands exist: `context7`, `github-readonly`, `browser`, `sentry-readonly`, `figma`, `db-dev`, `cloud-readonly`, and `cloud-write`.
