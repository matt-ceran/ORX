...
- Completed:
  ...
  - Add local plugin catalog editor commands: `orx plugins catalog add-local|add-git|remove` and `/plugins catalog add-local|add-git|remove` write private local catalog declarations with safe local path, pinned git source, tag, and description parsing and no install/enable/trust/grant/fetch/execution side effects.
