...
- Add top-level `orx doctor` readiness overview for runtime config, API-key presence, MCP, plugins, delegation, test targets, concrete next commands, concise `overall`/`ready_to_use`/surface readiness labels, and `orx doctor --strict` local ready-to-use exit gating without OpenRouter, remote MCP, bin, or hook execution.
