...
- Add tree-sitter AST preview command: `orx code tree-sitter <file>`, `orx tree-sitter <file>`, `/code tree-sitter <file>`, and `/tree-sitter <file>` run an installed local `tree-sitter parse` with shell disabled, cleaned env, cwd-confined file guards, bounded/redacted output, no network/install behavior, and no mutation; lexical code-map surfaces remain the fallback.
...
- Add overnight implementor/verifier dashboard harness: `npm run overnight:init`, `npm run overnight`, and `npm run overnight:dashboard` manage gitignored `.orx/overnight/latest` state/prompts/logs/events, render a fixed-screen sanitized dashboard, run implementor -> local checks -> verifier, require final-line `VERDICT: PASS` before optional `--commit`/`--push`, commit only paths newly changed during the verified slice, and keep `--require-clean` as an opt-in dirty-worktree preflight.
...
- Extend tree-sitter-backed code intelligence beyond the initial operator parse preview into richer syntax-aware call/reference/import slices when grammar/runtime constraints are acceptable.
