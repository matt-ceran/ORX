...
- Add top-level `orx doctor` readiness overview for runtime config, API-key presence, MCP, plugins, delegation, test targets, concrete next commands, and concise `overall`/`ready_to_use`/surface readiness labels without OpenRouter, remote MCP, bin, or hook execution.
