...
- Completed:
  ...
  - Add local plugin catalog inspect/editor commands: `orx plugins catalog inspect|add-local|add-git|remove` and `/plugins catalog inspect|add-local|add-git|remove` review or write private local catalog declarations with safe local path, pinned git source, tag, and description parsing and no install/enable/trust/grant/fetch/execution side effects.
...
- Extend the plugin system beyond the registry/CLI/cache/catalog/git-source/catalog-editor/scaffold/validate substrate with richer marketplace/catalog UX, update checks, expanded authoring docs/templates, and optional stronger provenance/signing.
