...
- Add default-reporter package-script JSON report files: when exact final no-reporter `jest`, `vitest`, or `playwright test` package-script invocations do not declare their own output file, ORX requests an ORX-owned temporary JSON report file, parses bounded counts before stdout/stderr fallback, and keeps wrapper commands, custom reporters, existing output files, per-run reporter/output overrides, and unsafe multi-step shapes on fallback.
...
- Extend structured test report ingestion beyond direct Node JUnit, JSON stdout/stderr, existing JSON reporter temp files, and exact default framework runner temp files to custom/configured reporters or wrapper commands when safe.
