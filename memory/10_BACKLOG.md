...
- Extend structured test report ingestion beyond direct Node JUnit, JSON stdout/stderr, JSON reporter temp files, exact default framework runner temp files, declared JSON output files, and config-declared JSON output files to non-JSON/custom reporters or wrapper commands when safe.
