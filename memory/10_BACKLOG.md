...
- Completed:
  ...
  - Add local plugin catalog editor commands: `orx plugins catalog add-local|remove` and `/plugins catalog add-local|remove` write private local catalog declarations with safe tag/description parsing and no install/enable/trust/grant/fetch/execution side effects.
