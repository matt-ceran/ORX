...
- Extend test adapters beyond direct Node JUnit, captured TAP/Mocha/pytest/Cargo/Deno/ExUnit/Gradle/JUnit-Platform/Go/RSpec/Minitest/Karma/Bun/Python-unittest/JUnit-text/PHPUnit/dotnet/CTest/XCTest-style summaries, and current JSON report package-script temp files, declared JSON output files, and config-declared JSON output files: cover additional non-JSON/custom reporters and wrapper commands only if this can be done without weakening execution/output bounds.
