...
- Extend test adapters beyond direct Node JUnit, captured TAP/Mocha/pytest/Cargo/Go/RSpec/Python-unittest/JUnit-text/PHPUnit-style summaries, and current JSON report package-script temp files, declared JSON output files, and config-declared JSON output files: cover additional non-JSON/custom reporters and wrapper commands only if this can be done without weakening execution/output bounds.
