...
Later diagnostics updates: local diagnostics profiles are implemented under `src/diagnostics/`. `orx diagnostics list|inspect`, `orx diagnostics run <typescript|pyright|eslint|gopls|clangd> [--project <local-project-path>] [--json]`, `orx diag run ...`, `/diagnostics ...`, and `/diag ...` expose operator-only TypeScript, Pyright, ESLint, gopls, and clangd runners with shell disabled, local cwd-confined project target guards, cleaned env, bounded/redacted output, parsed TypeScript text diagnostics, Pyright `generalDiagnostics`, ESLint JSON messages, gopls text diagnostics, or clangd stderr diagnostics, no install/network behavior, gopls proxy/checksum/toolchain downloads pinned off, no model-tool exposure, and catalog-only TypeScript Language Server/rust-analyzer/SCIP TypeScript readiness profiles. gopls specifically requires `--project <local-go-file>`, probes PATH with `gopls version`, and runs `gopls check <go-file>` against a regular cwd-confined `.go` file. Future LSP/SCIP work should extend this boundary rather than replacing it.
...
- Extend diagnostics beyond the current TypeScript/Pyright/ESLint/gopls/clangd CLI adapters with LSP/SCIP bridges for diagnostics, references, hover, and go-to-definition.
