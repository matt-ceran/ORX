...
Later 2026-06-30 update: local diagnostics profiles are implemented under `src/diagnostics/`. `orx diagnostics list|inspect`, `orx diagnostics run <typescript|pyright> [--project <local-project-path>] [--json]`, `orx diag run ...`, `/diagnostics ...`, and `/diag ...` expose operator-only TypeScript and Pyright runners with shell disabled, local cwd-confined project target guards, cleaned env, bounded/redacted output, parsed TypeScript text diagnostics or Pyright `generalDiagnostics`, no install/network behavior, no model-tool exposure, and catalog-only TypeScript Language Server/rust-analyzer/gopls/clangd/SCIP TypeScript readiness profiles. Future LSP/SCIP work should extend this boundary rather than replacing it.
...
- Extend diagnostics beyond the current TypeScript/Pyright CLI adapters with LSP/SCIP bridges for diagnostics, references, hover, and go-to-definition.
