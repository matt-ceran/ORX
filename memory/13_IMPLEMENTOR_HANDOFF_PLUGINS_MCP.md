...
Later 2026-06-30 update: local diagnostics profiles are implemented under `src/diagnostics/`. `orx diagnostics list|inspect`, `orx diagnostics run <typescript|pyright|gopls> [--project <local-project-path>] [--json]`, `orx diag run ...`, `/diagnostics ...`, and `/diag ...` expose operator-only TypeScript, Pyright, and gopls runners with shell disabled, local cwd-confined project target guards, cleaned env, bounded/redacted output, parsed TypeScript text diagnostics, Pyright `generalDiagnostics`, or gopls text diagnostics, no install/network behavior, gopls proxy/checksum/toolchain downloads pinned off, no model-tool exposure, and catalog-only TypeScript Language Server/rust-analyzer/clangd/SCIP TypeScript readiness profiles. gopls specifically requires `--project <local-go-file>`, probes PATH with `gopls version`, and runs `gopls check <go-file>` against a regular cwd-confined `.go` file. Future LSP/SCIP work should extend this boundary rather than replacing it.
