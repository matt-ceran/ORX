...
NUnit XML captured-output parsing note: already-captured whole `<test-run>` NUnit 3 XML streams now render `source=nunit-xml` counts through the same stdout/stderr fallback path. The parser validates strict root `total`/`passed`/`failed` plus optional `warnings`/`inconclusive`/`skipped`/`testcasecount` aggregate counts, folds warnings into passed and inconclusive into skipped to match NUnit console summary parsing, converts optional root `duration` seconds to ms, rejects malformed or impossible whole NUnit XML before text/generic fallbacks, leaves mixed logs plus XML on existing text fallback, and does not add reporter flags, report files, installs, invocation changes, network, or new model-tool exposure.
