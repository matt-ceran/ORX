...
Later 2026-06-30 update: initial local diagnostics profiles are implemented under `src/diagnostics/`. `orx diagnostics list|inspect`, `orx diagnostics run typescript [--project <local-tsconfig-path>] [--json]`, `orx diag run typescript ...`, `/diagnostics ...`, and `/diag ...` expose an operator-only TypeScript compiler runner with shell disabled, local cwd-confined project-file guards, cleaned env, bounded/redacted output, parsed diagnostic summaries, no install/network behavior, no model-tool exposure, and catalog-only TypeScript Language Server/Pyright/rust-analyzer/gopls/clangd/SCIP TypeScript readiness profiles. Future LSP/SCIP work should extend this boundary rather than replacing it.
...
- Extend diagnostics beyond the initial TypeScript compiler adapter with LSP/SCIP bridges for diagnostics, references, hover, and go-to-definition.
