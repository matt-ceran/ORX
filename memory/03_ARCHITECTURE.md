...
`src/plugins/` now owns the inert plugin substrate plus progressive text-component loaders: sanitized ORX plugin manifests, optional display-only manifest metadata, stable plugin ids, manifest hashes, local lock-style records, local component hashes, ORX-owned install cache, local catalog metadata, private registry persistence, installed/enabled state separation, `/plugins` rendering, bounded enabled-plugin-only `SKILL.md` discovery, bounded markdown prompt/rule discovery, compact metadata rendering, and explicit skill/prompt/rule activation. Plugin enablement is only a persisted state marker plus metadata eligibility in this scaffold; hooks, bins, executable plugin commands, plugin MCP servers, and plugin code execution remain inactive.
...
The next plugin work should add remote source semantics, plugin-provided MCP presets, executable command design, and hook trust without changing the current trust boundary.
