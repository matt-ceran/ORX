...
`src/plugins/` now owns the inert plugin substrate plus progressive text-component loaders: sanitized ORX plugin manifests, optional display-only manifest metadata, stable plugin ids, manifest hashes, local lock-style records, local component hashes, ORX-owned install cache, local catalog metadata, private registry persistence, installed/enabled state separation, `/plugins` rendering, bounded enabled-plugin-only `SKILL.md` discovery, bounded markdown prompt/rule discovery, compact metadata rendering, render-only plugin MCP preset discovery, hook discovery/trust state, trusted-hook manual runtime, and explicit skill/prompt/rule activation. Plugin enablement is only a persisted state marker plus metadata/preset/review eligibility; trusted current hooks run only through explicit `hooks run` / `/hooks run`, while automatic lifecycle hook execution, bins, executable plugin commands, plugin endpoint discovery, plugin MCP tool execution, and other plugin code execution remain inactive.

`src/mcp/` now merges the built-in OpenRouter MCP profile with enabled-plugin MCP preset declarations when a plugin registry path is supplied. Plugin MCP profile ids are namespaced as `plugin:<plugin-id>:<server-id>`, include plugin manifest/component provenance in profile hashes, and flow through the existing `/mcp list`, `/mcp inspect`, `/mcp tools`, `/mcp enable`, `/status`, and schema-change policy surfaces. `/mcp discover` intentionally refuses plugin-sourced profiles until plugin endpoint/network trust is designed.
...
The next plugin work should add remote source semantics, executable command design, hook execution policy, and a separate runtime policy for plugin MCP endpoint discovery/tool execution without changing the current trust boundary.
