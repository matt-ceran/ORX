...
src/code-map/              bounded local repository overview and exported-symbol summaries
...
`src/code-map/` owns operator-invoked local code-map and symbol-index rendering. It scans a bounded local tree, skips generated/vendor directories, summarizes languages, key files, package/config/source entrypoints, top JavaScript/TypeScript imports/exports, and exported symbols with file paths and line numbers, and redacts secret-like rendered paths or symbols. It is exposed through `orx code map`, `orx map`, `orx code-map`, `orx code symbols`, `orx symbols`, `/map`, `/code map`, `/code symbols`, and `/symbols` without requiring an OpenRouter API key.
...
`src/mcp/` now merges the built-in OpenRouter MCP profile with enabled-plugin MCP preset declarations when a plugin registry path is supplied. Plugin MCP profile ids are namespaced as `plugin:<plugin-id>:<server-id>`, include plugin manifest/component provenance in profile hashes, and flow through the existing `/mcp list`, `/mcp inspect`, `/mcp tools`, `/mcp call`, `/mcp remote-tools`, `/mcp enable`, `/mcp discover`, `/status`, and schema-change policy surfaces. `/mcp discover` can contact enabled, trusted, unchanged plugin `remote-http` endpoints through ORX's guarded DNS-vetted discovery transport for a minimal initialize handshake. `/mcp remote-tools` can call `tools/list` through the same guarded transport, render bounded untrusted metadata plus schema hashes, and audit the result. `/mcp call` and `orx mcp call` can execute guarded `tools/call` through explicit operator commands. Interactive `/mcp model enable` and one-shot `orx ask --mcp-tools` add one ORX-owned native model tool, `mcp_call`, limited to read-only non-billable declared MCP tools with active model-tool grants through the same profile/hash/auth/transport gates.
...
The next programming power-pack work should add richer code intelligence such as tree-sitter-backed call/reference slices, ast-grep previews, LSP/SCIP diagnostics, or structured framework report ingestion beyond the current summary-line parsing in the framework-aware package-script/Node test runner, model-visible `run_tests`, dependency-free code map, and exported-symbol index.
