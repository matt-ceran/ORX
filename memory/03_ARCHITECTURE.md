...
`src/mcp/` now merges the built-in OpenRouter MCP profile with enabled-plugin MCP preset declarations when a plugin registry path is supplied. Plugin MCP profile ids are namespaced as `plugin:<plugin-id>:<server-id>`, include plugin manifest/component provenance in profile hashes, and flow through the existing `/mcp list`, `/mcp inspect`, `/mcp tools`, `/mcp enable`, `/mcp discover`, `/status`, and schema-change policy surfaces. `/mcp discover` can contact enabled, trusted, unchanged plugin `remote-http` endpoints through ORX's guarded DNS-vetted discovery transport, but it only performs the minimal initialize handshake and does not list, execute, or expose plugin MCP tools.
...
The next plugin work should add remote source semantics, executable command design, and a separate runtime policy for plugin MCP tool listing/execution without changing the current trust boundary.
