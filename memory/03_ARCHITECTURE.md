...
`src/plugins/` now owns the inert plugin substrate plus progressive text-component loaders: sanitized ORX plugin manifests, optional display-only manifest metadata, stable plugin ids, manifest hashes, local lock-style records, local component hashes, ORX-owned install cache, local catalog metadata, private registry persistence, installed/enabled state separation, `/plugins` rendering, bounded enabled-plugin-only `SKILL.md` discovery, bounded markdown prompt/rule discovery, compact metadata rendering, render-only plugin MCP preset discovery, and explicit skill/prompt/rule activation. Plugin enablement is only a persisted state marker plus metadata/preset eligibility in this scaffold; hooks, bins, executable plugin commands, plugin endpoint discovery, plugin MCP tool execution, and plugin code execution remain inactive.
...
The next plugin work should add remote source semantics, executable command design, hook trust, and a separate runtime policy for plugin MCP endpoint discovery/tool execution without changing the current trust boundary.
