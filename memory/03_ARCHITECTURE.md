...
src/code-map/              bounded local repository overview, symbols, references, and import graphs
...
`src/code-map/` owns operator-invoked local code-map, symbol-index, reference-index, and import-graph rendering. It scans a bounded local tree, skips generated/vendor directories, summarizes languages, key files, package/config/source entrypoints, top JavaScript/TypeScript imports/exports, exported symbols with file paths and line numbers, code-reference matches, and local import edges with relative import resolution where possible, and redacts secret-like rendered paths, symbols, and excerpts. It is exposed through `orx code map`, `orx map`, `orx code-map`, `orx code symbols`, `orx symbols`, `orx code refs`, `orx refs`, `orx code imports`, `orx imports`, `/map`, `/code map`, `/code symbols`, `/symbols`, `/code refs`, `/refs`, `/code imports`, and `/imports` without requiring an OpenRouter API key.
...
The next programming power-pack work should add richer code intelligence such as tree-sitter-backed call/reference/import slices, ast-grep previews, LSP/SCIP diagnostics, or structured framework report ingestion beyond the current summary-line parsing in the framework-aware package-script/Node test runner, model-visible `run_tests`, dependency-free code map, exported-symbol index, reference index, and import graph.
