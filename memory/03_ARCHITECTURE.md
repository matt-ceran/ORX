...
The next programming power-pack work should add richer code intelligence such as tree-sitter-backed call/reference/import slices, LSP/SCIP diagnostics, or structured framework report ingestion beyond the current summary-line parsing in the framework-aware package-script/Node test runner, model-visible `run_tests`, dependency-free code map, exported-symbol index, reference index, import graph, lexical call graph, and optional ast-grep adapter.
