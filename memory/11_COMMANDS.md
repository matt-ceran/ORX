...
npm run dev -- scanners setup-plan osv-scanner --json
...
Additional scanner slash examples: `/scanners inspect trivy`, `/scanners plan semgrep`, `/scanners setup-plan osv-scanner --json`, `/scanners run trivy src`, `/scan trivy src --json`, `/scanners run codeql codeql-db --query queries/example.ql`, and `/scan codeql codeql-db --query queries/example.ql --json`.
