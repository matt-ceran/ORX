...
npm run dev -- web fetch https://example.com
npm run dev -- web search openrouter models
npm run dev -- web browse https://example.com
npm run dev -- research profiles
npm run dev -- research profiles --json
npm run dev -- research profiles inspect research-web
npm run dev -- research profiles plan research-rag --json
npm run dev -- scanners list
npm run dev -- scanners inspect semgrep
npm run dev -- scanners inspect trivy
npm run dev -- scanners plan semgrep
...
npm run dev -- scanners run codeql codeql-db --query queries/example.ql
npm run dev -- scan codeql codeql-db --query queries/example.ql --json
npm run dev -- scanners run osv-scanner src
npm run dev -- scan osv-scanner src --json
...
npm run dev -- plugins validate ./my-plugin --json
...
`npm run verify:release` is the local v0.1 release-boundary gate. It clears real operator API/search keys, runs `git diff --check`, `npm run typecheck`, full `npm test`, `npm run verify:global-install`, npm package dry-run content assertions, and built CLI smokes for `doctor --json`, `guide`, `code calls`, `plugins review`, `plugins review --json`, `plugins validate --json`, and `mcp presets` against temporary isolated ORX state. The package assertion requires runtime docs/bin files and rejects source, memory, scripts, local ORX state, lockfiles, and compiled test/spec artifacts. It is no-real-key/no-network by command selection: the nested global-install chat-launch smoke uses only a non-secret placeholder key to start chat and immediately run `/exit`, and the gate does not call OpenRouter, remote MCP endpoints, plugin bins, or plugin hooks. Use `npm run pack:dry-run` to inspect package contents without publishing; v0.1 release handoff notes live in `RELEASE_NOTES.md`.
...
Web research commands are explicit operator commands, not model-autonomous browsing. `/web profiles [list|inspect|plan] ...`, `orx web profiles ...`, and `orx research profiles ...` render ORX-owned read-only profile metadata for `research-web`, `research-browser`, `research-crawl`, `research-scholar`, `research-docs`, `research-rag`, and `research-memory` without fetching URLs, calling search providers, launching browsers, spawning processes, writing state, or exposing model tools. `orx web fetch <url>`, `orx research fetch <url>`, `/web fetch <url>`, and `/fetch <url>` fetch guarded URLs directly. `orx web search <query>`, `orx research search <query>`, `/web search <query>`, and `/search <query>` call Brave Web Search only when `BRAVE_SEARCH_API_KEY` is configured; results are secondary provider-snippet evidence and primary pages are not fetched. `orx web browse <url>`, `orx research browse <url>`, `/web browse <url>`, and `/browse <url>` create browser evidence snapshots when Playwright is available, using URL/DNS/final-URL guards before recording output. Noninteractive CLI forms print source metadata and explicitly do not append model context or write session evidence state; chat slash forms keep the session evidence ledger, `/sources`, `/cite`, and `/bibliography`. Crawl, scholar, docs, RAG, and durable memory profiles remain catalog-only until their source, budget, storage, citation, and model-visible retrieval contracts are implemented.
