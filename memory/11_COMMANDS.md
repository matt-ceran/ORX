...
Delegation CLI commands are no-key and non-executing. `orx orchestrator`, `orx orchestrator plan`, `orx delegate`, `orx delegate plan`, `orx delegates`, and `orx delegates plan` render the inert scaffold status plus readiness blockers. Live-session forms such as `orx orchestrator openrouter <model>`, `orx orchestrator clear`, `orx delegate add <name> openrouter <model>`, `orx delegate remove <name>`, and `orx delegate clear` validate safe names/models and then refuse because noninteractive CLI has no active chat session to mutate. Future delegation execution policy can be inspected or tuned with `orx delegates policy` / `orx delegate policy` and `policy set --max-cost-usd <n> --timeout-ms <ms> --max-result-bytes <bytes> --max-concurrent <n> --credentials none --result-persistence none --result-merge manual_summary`; policy storage remains disabled and inert. The internal `delegate_task` runtime contract is implemented but hidden from normal model requests; explicit dispatch returns a fail-closed envelope and writes hash-only audit metadata to `~/.orx/audit/delegation.jsonl` or `ORX_DELEGATION_AUDIT_PATH`. Saved disabled teams can be managed with `orx delegates teams`, `orx delegates save <id> --controller <model> [--delegate <name> <model>...]`, `orx delegates inspect <id>`, `orx delegates use <id>`, `orx delegates delete <id>`, and `orx delegate team ...`; CLI `use` is read-only. These commands do not call OpenRouter, spawn subprocess agents, enable execution, or expose `delegate_task` to normal chat models.
...
Orchestration slash commands are scaffolds only. `/orchestrator` shows local controller state, `/orchestrator plan` renders readiness blockers, `/orchestrator openrouter <model>` stores an inert OpenRouter controller, and `/orchestrator clear` clears the controller. `/delegate status` and `/delegate plan` render inert delegate state/readiness, `/delegate add <name> openrouter <model>` stores an inert named delegate, `/delegate remove <name>` and `/delegate clear` remove delegates, `/delegate policy` and `/delegates policy` show or tune inert future execution limits, and `/delegates [list|status|plan|policy|teams|save|use|inspect|delete]` lists delegates/readiness or manages saved disabled teams. `/delegate team save <id>` captures the current inert chat scaffold and `/delegate team use <id>` loads a saved disabled team into the current chat session. No external delegate calls, subprocess agents, normal model-visible `delegate_task`, or delegated result merge exist yet.
