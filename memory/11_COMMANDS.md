...
Command discovery also has `/commands [query]`, with `/palette` as an alias. In TTY/color-capable chat this renders a compact command palette; in non-TTY and `NO_COLOR=1` it renders the deterministic grouped plain palette. Interactive readline sessions complete slash command names and aliases with Tab, for example `/sta<Tab>` -> `/status `. They also complete deterministic subcommands/arguments such as `/mode a<Tab>` -> `auto`, `/auth e<Tab>` -> `env`/`env-file`, `/web h<Tab>` -> `help`, `/tests r<Tab>` -> `run`, `/code m<Tab>` -> `map`, `/code s<Tab>` -> `symbols`, `/code i<Tab>` -> `imports`, `/code c<Tab>` -> `calls`, `/code a<Tab>` -> `ast-grep`, `/code t<Tab>` -> `tree-sitter`, `/code o<Tab>` -> `outline`, `/code tree-sitter o<Tab>` -> `outline`, `/scanners r<Tab>` -> `run`, `/scan s<Tab>` -> `semgrep`, `/diagnostics r<Tab>` -> `run`, `/diag run typescript --p<Tab>` -> `--project`, and `/mcp inspect o<Tab>` -> `openrouter`.
