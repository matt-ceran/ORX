...
Added safe root tsconfig path/baseUrl resolution for tree-sitter repo deps:

- `repo-deps` now reads a small regular root `tsconfig.json` when present, accepts JSONC-style comments/trailing commas, and resolves safe `compilerOptions.paths` and `baseUrl` aliases only to files already inside the bounded scanned source set.
- Rendered dependency edges include `resolution_detail=relative`, `resolution_detail=tsconfig_path`, or `resolution_detail=tsconfig_base_url` for local edges. Unsafe alias targets are skipped with warnings, matched aliases with no scanned target count as `unresolved_local`, and unmatched package imports remain `external`.
...
- Verification: `npm run typecheck`, focused `npm run build && node --test dist/code-map/code-map.test.js`, `git diff --check`, full `npm test` with 540 tests, `npm run verify:release`, and independent verifier review/recheck passed. The verifier found and rechecked a tsconfig `paths` precedence overclaim; `repo-deps` now stops at the first matching path pattern instead of falling through to broader aliases.
