...
- Built-in MCP provider presets are implemented through `orx mcp presets [--json]`, `orx mcp presets search|find <query> [--json]`, `orx mcp presets inspect <preset> [--json]`, `orx mcp add-preset <preset>`, `/mcp presets [--json]`, `/mcp presets search|find <query> [--json]`, `/mcp presets inspect <preset> [--json]`, and `/mcp add-preset <preset>`. Templates now include `context7`, `deepwiki`, `microsoft-learn`, `github-readonly`, `github-write`, `gitlab-readonly`, `gitlab-ci-write`, `sourcegraph-github-readonly`, `sentry-readonly`, `figma`, `atlassian-rovo`, `linear`, `notion`, `browser`, `cloudflare-docs`, and `cloudflare-api`. Preset list/search/inspect JSON emits ORX-owned structured declaration metadata only, with no install, enable, trust, grant, fetch, call, audit, model exposure, network, execution, or catalog-write side effects; install flows still create disabled local user profiles only and leave enablement, trust, grants, calls, and model exposure as separate explicit steps.
...
- Added built-in provider presets `atlassian-rovo` (`https://mcp.atlassian.com/v1/mcp/authv2`), `linear` (`https://mcp.linear.app/mcp`), and `notion` (`https://mcp.notion.com/mcp`) after checking current official provider docs.
- Each profile is auth-required, high-risk, write-capable, disabled on install, and declares no static tools until the operator enables/trusts the profile, reviews remote tool metadata, declares tools explicitly, grants tools explicitly, and opts into any model exposure separately.
- Added provider-specific no-network auth setup guidance for exact Atlassian, Linear, and Notion MCP endpoints with write-capable scope warnings. ORX still does not run OAuth/browser/device flows or persist token values.
- Added focused MCP regressions for disabled profile installation, high/write-capable markers, no static tools, local metadata search discoverability, provider auth setup guidance, no network calls, and no bearer leakage.
- Verification: typecheck, focused MCP preset/auth tests, build, broader `src/mcp/mcp.test.ts src/cli.test.ts src/slash/index.test.ts`, built CLI inspect/plan/add-preset/auth-setup smokes for the three new presets, `git diff --check`, and `npm run verify:release` all pass. Independent verifier first found non-default-port exact-endpoint spoofing and command-memory ordering issues; after fixes, scoped verifier recheck passed.
- Next likely work remains another bounded optional completion slice such as actual LSP/SCIP execution/readback after safe local lifecycle/storage design, Snyk/Socket adapters after deterministic no-network/no-auth command shapes are proven, reviewed provider tool declaration packs after remote metadata inspection, or making one catalog-only research profile executable after its source/storage/citation contract is designed.
...
- Search rejects missing, dash-prefixed option-like, control-character, overlong, and secret-like queries without echoing secret-shaped input. Text and JSON output include authority metadata showing local built-in preset metadata only, no execution, no network, no catalog writes, no install/enable/trust/grant/fetch/call/audit/model exposure, and no model tool.
- Added CLI/slash completions and regressions for text search, JSON search, no-match next-step output, usage failures, and secret-query redaction.
...
- Promoted `osv-scanner` from scanner catalog-only metadata to a runnable explicit-operator profile through `orx scanners run osv-scanner <path> [--json]`, `orx scan osv-scanner ...`, `/scanners run osv-scanner ...`, and `/scan osv-scanner ...`.
- The command shape is fixed to an already-installed local `osv-scanner` binary running `scan source --recursive --format json --offline --no-resolve <path>` against a cwd-confined regular file or directory. ORX does not pass `--download-offline-databases`, accept `--config` or `--query`, enable online matching, run image/license/fix modes, install scanners, call network by command selection, write ORX/project state, or expose scanner runs as model tools.
- Added CLI and slash regressions for catalog/list/inspect/plan state, fixed OSV argv, shell-disabled/minimal-env execution, redacted text and JSON output, unsupported `--config`/`--query` refusal before spawn, and symlink target escape refusal before spawn.
- Verification: `npm run typecheck`, focused source scanner/slash tests, `npm run build`, built CLI fake-OSV smoke for `--offline --no-resolve` argv/no vulnerability-only offline flag/no download flag/config rejection before spawn/cleaned env/redaction, full CLI/slash source tests (151 passed), `git diff --check`, `npm run verify:release`, and independent read-only verifier `019f20f9-2c49-7e43-92cc-19eb872c14c9` PASS after tightening the earlier weaker vulnerability-only offline shape to OSV's full `--offline` mode.
- Next likely work remains another bounded optional completion slice such as actual LSP/SCIP execution/readback after safe local lifecycle/storage design, Snyk/Socket adapters after deterministic no-network/no-auth command shapes are proven, provider preset/tool declaration packs after current docs/metadata review, or making one catalog-only research profile executable after its source/storage/citation contract is designed.

Previous latest work:

Extended profile-independent dispatch to state/metadata namespaces:

- Moved `orx history`, `orx plugins`/`plugin`, `orx bins`/`bin`, `orx hooks`/`hook`, `orx mcp`, `orx orchestrator`, `orx delegate`, and `orx delegates` after their own path resolution but before config/profile loading.
- These commands still use their own explicit state paths and policies, but global `--profile <id>` no longer reads or chmods the saved-profile registry, and malformed ORX config does not block these profile-independent metadata/state commands.
- Added a table-style CLI regression with an invalid config file plus a `0666` `ORX_PROFILE_CONFIG_PATH` file for history, plugins review, bins list, hooks list, mcp list, orchestrator status, delegate status, and delegates status.
- Verification passed: isolated-home `npm run typecheck`, `npm run build`, focused `node --import tsx --test --test-name-pattern "profile-independent state|history|plugins install list|hooks list|bins list|mcp commands manage|delegation commands|cli tests commands|cli code map" src/cli.test.ts`, isolated-home `node --import tsx --test src/cli.test.ts src/slash/index.test.ts` with 151 tests, built CLI invalid-config/no-chmod smokes for history/plugins/bins/hooks/MCP/delegation status commands, `git diff --check`, isolated-home `npm run verify:release` with all 12 steps passing, and independent verifier `019f20e2-486b-7243-bf55-56190880dee5` PASS with no findings.
...
Extended profile-independent dispatch to tests and code intelligence:

- Moved `orx tests`, `orx test`, `orx code`, and the top-level local code-intelligence aliases (`map`, `code-map`, `symbols`, `refs`, `references`, `imports`, `import-graph`, `calls`, `call-graph`, `ast-grep`, `tree-sitter`, and `outline`) before config/profile loading.
- These local-only commands now match the `research`, `scanners`, `scan`, `diagnostics`, and `diag` boundary: global `--profile <id>` is parsed but does not read or chmod the saved-profile registry when running profile-independent local metadata or explicit local tooling.
- Added CLI regressions proving `orx --profile demo tests list --json` and `orx --profile demo code map --json` leave an existing `ORX_PROFILE_CONFIG_PATH` file mode unchanged.
- Verification passed: isolated-home `npm run typecheck`, `npm run build`, focused `node --import tsx --test --test-name-pattern "cli tests commands|cli code map|scanner commands|diagnostics commands|research profile" src/cli.test.ts`, isolated-home `node --import tsx --test src/cli.test.ts src/slash/index.test.ts` with 150 tests, built CLI no-chmod smokes for tests and code map, `git diff --check`, isolated-home `npm run verify:release` with all 12 steps passing, and independent verifier `019f20d6-8b29-7b52-93f9-0c1aa34e207d` PASS with no findings.
- Next likely work remained another bounded optional completion slice such as actual LSP/SCIP execution/readback after safe local lifecycle/storage design, scanner adapters after deterministic no-network/no-auth command shapes are proven, provider preset/tool declaration packs after current docs/metadata review, or making one catalog-only research profile executable after its source/storage/citation contract is designed.

Earlier latest work:

Hardened profile-independent local tooling dispatch:

- Moved `orx scanners`, `orx scan`, `orx diagnostics`, and `orx diag` dispatch before config/profile loading, matching the already-applied `orx research` boundary.
- Global `--profile <id>` is still parsed by the CLI, but these local tooling namespaces do not read or chmod the saved-profile registry because the saved routing profile has no effect on their list/inspect/plan/run behavior.
- Added CLI regressions proving `orx --profile demo scanners list --json` and `orx --profile demo diagnostics list --json` leave an existing `ORX_PROFILE_CONFIG_PATH` file mode unchanged.
- Verification passed: isolated-home `npm run typecheck`, `npm run build`, focused `node --import tsx --test --test-name-pattern "research profile|scanner commands|diagnostics commands" src/cli.test.ts src/slash/index.test.ts`, isolated-home `node --import tsx --test src/cli.test.ts src/slash/index.test.ts` with 150 tests, built CLI no-chmod smokes for scanners and diagnostics, `git diff --check`, isolated-home `npm run verify:release` with all 12 steps passing, and independent verifier `019f20cf-9e27-74b2-8680-9b533345260f` PASS with no findings.
- Next likely work remained another bounded optional completion slice such as actual LSP/SCIP execution/readback after safe local lifecycle/storage design, scanner adapters after deterministic no-network/no-auth command shapes are proven, provider preset/tool declaration packs after current docs/metadata review, or making one catalog-only research profile executable after its source/storage/citation contract is designed.
...
- At that point, OSV-Scanner remained catalog-only because ORX still needed an explicit offline/update/cache/write/readback contract before running it; OSV-Scanner has since been promoted to a guarded offline source/lockfile runnable profile. Snyk and Socket remain catalog-only until deterministic local no-auth/no-network command shapes and denial tests are designed.
...
- Semgrep was the only runnable profile in that initial slice. Trivy was later promoted to a filesystem-secret-only runnable profile, CodeQL was later promoted to a local database-analysis runnable profile, and OSV-Scanner was later promoted to a guarded offline source/lockfile runnable profile; Snyk and Socket remain catalog/readiness-only until no-network/no-auth local command shapes are proven.
