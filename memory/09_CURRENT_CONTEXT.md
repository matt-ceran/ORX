...
- Jest and Vitest receive only `--outputFile=<private-temp-json>` as package-script passthrough args; Playwright receives `PLAYWRIGHT_JSON_OUTPUT_FILE=<private-temp-json>` in the child env. ORX does not force JSON reporters onto default package scripts.
...
- Next likely work after this slice is another optional completion slice such as broader default-reporter framework report-file requests, tree-sitter-backed syntax-aware code intelligence, LSP/SCIP diagnostics/references, or final v0.1 packaging/release polish.
...
- Next likely work after this slice is another optional completion slice such as tree-sitter-backed syntax-aware code intelligence, LSP/SCIP diagnostics beyond the initial TypeScript compiler profile, broader default-reporter framework report-file requests, Sourcegraph/GitHub read-only profiles, or final v0.1 packaging/release notes.
...
- Next likely programming-power-pack work: tree-sitter-backed code intelligence or broader default-reporter framework report-file requests when ORX can safely request them.
