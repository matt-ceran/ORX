...
- Existing JSON reporter scripts still receive only output-file wiring where append-safe; wrapper commands such as `react-scripts test`, custom `node ... jest` runners, custom reporters, existing output files, per-run reporter/output overrides, and post-step script shapes stay on stdout/stderr fallback.
...
- Next likely work after this slice is another optional completion slice such as tree-sitter-backed syntax-aware code intelligence, LSP/SCIP diagnostics/references, custom/configured reporter integration, or final v0.1 packaging/release polish.
...
- Next likely work after this slice is another optional completion slice such as custom/configured reporter integration, tree-sitter-backed syntax-aware code intelligence, LSP/SCIP diagnostics/references, or final v0.1 packaging/release polish.
...
- Command discovery, README, guide, command memory, architecture/tooling notes, release notes, and backlog now show TypeScript plus Pyright as runnable diagnostics profiles; TypeScript Language Server, rust-analyzer, gopls, clangd, and SCIP TypeScript remain catalog/readiness-only.
...
- TypeScript was the only runnable profile in this initial slice. Pyright was later promoted to a runnable local profile; TypeScript Language Server, rust-analyzer, gopls, clangd, and SCIP TypeScript remain catalog/readiness-only until a no-network/no-auth local command shape is proven.
...
- Next likely work after this slice is another optional completion slice such as tree-sitter-backed syntax-aware code intelligence, LSP/SCIP diagnostics beyond the initial TypeScript compiler profile, custom/configured reporter integration, Sourcegraph/GitHub read-only profiles, or final v0.1 packaging/release notes.
...
- Next likely programming-power-pack work: tree-sitter-backed code intelligence or custom/configured reporter integration when ORX can safely request it.
