...
- TypeScript is the only runnable profile in this slice. TypeScript Language Server, Pyright, rust-analyzer, gopls, clangd, and SCIP TypeScript are catalog/readiness-only until a no-network/no-auth local command shape is proven.
