...
- The internal `delegate_task` runtime contract is implemented but not exposed to normal model requests. It validates arguments against the disabled policy, resolves only sanitized disabled OpenRouter delegates, returns a fail-closed result envelope, writes hash-only audit metadata to `~/.orx/audit/delegation.jsonl` or `ORX_DELEGATION_AUDIT_PATH`, and never calls OpenRouter delegates, spawns subprocesses, persists outputs, or merges delegated results.
...
- Next likely delegation work from the current baseline: wire the OpenRouter delegate adapter behind the now-existing policy/audit/result-envelope contract, then separately enable normal model-visible `delegate_task` only after live enforcement and merge boundaries are verified.
...
- The CLI renders inert scaffold status plus readiness blockers for future execution: normal model exposure is still unavailable, the OpenRouter adapter is still missing, live execution remains disabled, result merge/persistence are not implemented, and CLI session mutation remains unavailable.
