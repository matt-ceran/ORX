...
- Built-in MCP provider presets are implemented through `orx mcp presets`, `orx mcp presets inspect <preset>`, `orx mcp add-preset <preset>`, `/mcp presets`, `/mcp presets inspect <preset>`, and `/mcp add-preset <preset>`. Templates now include `context7`, `microsoft-learn`, `github-readonly`, `sourcegraph-github-readonly`, `sentry-readonly`, `figma`, `browser`, `cloudflare-docs`, and `cloudflare-api`, and inspect/install flows leave enablement, trust, grants, calls, and model exposure as separate explicit steps.
...
- Next likely work after this slice remains another bounded optional completion slice such as LSP/SCIP diagnostics/references, semantic tree-sitter refs/dependency depth, non-JSON/custom reporter parsing, Sourcegraph/GitHub read-only profiles, or scanner adapters/additional Trivy modes only after deterministic no-network/no-auth local command shapes are proven.
...
- Readiness JSON includes `surface`, `schema_version`, explicit operator/model/network boundary fields, profile state/binary/run support, install behavior, network boundary, and runner-specific guard details for runnable TypeScript/Pyright/ESLint/Ruff/gopls/clangd profiles. Extra readiness operands and unknown readiness options reject with usage; slash completion now covers `--json` for list/status and inspect/show.
...
- Semgrep was the only runnable profile in that initial slice. Trivy was later promoted to a filesystem-secret-only runnable profile; Snyk, Socket, OSV-Scanner, and CodeQL remain catalog/readiness-only until no-network/no-auth local command shapes are proven.
