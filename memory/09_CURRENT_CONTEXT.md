...
- TTY chat transcript presentation now uses clearer user/assistant blocks, sanitizes terminal controls in displayed TTY transcript chunks, and renders TTY tool calls/results as compact multi-line product blocks. Non-TTY and `NO_COLOR=1` still keep the plain line-oriented transcript and one-line `[tool]` summaries.
- A small `scripts/probe-tty-render.mjs` render probe snapshots representative built TTY transcript/status output at 80 and 120 columns; browser/pixel terminal verification remains a later phase.
...
- The TTY bottom HUD product pass is implemented: context and observed-cost meters now sit in the bottom notch with frame-driven pulse markers during assistant/tool activity, cost shows metadata coverage as a real meter, permissions remain visible but secondary, and `renderTtyStatusProbe(width)` provides pure 80/120/narrow render probes. True PTY/xterm/browser pixel QA remains a later visual harness.
- The TTY screen-controller pass is implemented: interactive chat now renders a framed `╰─ orx ›` composer, tracks the visible composer block, clears submitted HUD/composer lines before scrollback, and redraws idle/activity composers in place so launch and assistant/tool work no longer leave repeated status blocks. Non-TTY and `NO_COLOR=1` remain line-oriented.
...
- Next likely task is a dedicated TTY visual QA harness, optional raw-mode input editor, or another bounded UI polish slice; keep using bounded implementor/verifier loops and commit/push each verified step before starting the next one.
...
- Next likely task is a dedicated TTY visual QA harness, composer/input ergonomics polish, or another small no-network structured-output/onboarding-polish slice; keep using bounded implementor/verifier loops and commit/push each verified step before starting the next one.
...
Implemented TTY screen-controller pass:

- Added a TTY screen controller that owns the visible HUD/composer block, clears it after submitted input, and redraws idle/activity composers in place instead of appending repeated status blocks to scrollback.
- The TTY composer is now framed as `╰─ orx ›`, with cwd/session/permissions kept in the status notch above it. The launch/idle state shows the compact bottom-oriented HUD rather than the old `ORX chat` header/footer.
- Extended `scripts/probe-tty-render.mjs` to show launch/idle output, escaped redraw traces, and assistant-turn frames for 80- and 120-column terminals.
- Verification passed so far: focused source TTY/terminal tests and `npm run typecheck`; build, full release checks, and independent verifier are pending.
- Next likely task is independent verification, build/full test closeout, and then optional xterm/pixel visual QA or raw-mode editor work if the current readline-backed controller is still not polished enough.

Previous latest work:

Implemented bottom HUD product pass:

- Refined the TTY bottom status notch/composer as the central product surface: route shortcuts remain compact, exact `provider/model` ids split once there is room, cwd/session stay secondary, and `never/danger-full-access` remains visible without being the loudest badge.
- Context and observed-cost meters now render together in the bottom HUD. Context is still approximate local bytes, cost is still only observed OpenRouter metadata, and active assistant/tool frames add subtle pulse markers without inventing usage data.
- Added `renderTtyStatusProbe(width)` plus focused source tests/snapshots for 80, 120, and very narrow widths; tests assert line widths, essential labels, NO_COLOR behavior, and route/model badge behavior.
- This slice did not add Playwright, xterm, or browser screenshot verification. True pixel/PTY visual QA remains a later harness.
