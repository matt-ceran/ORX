...
- Readiness JSON includes `surface`, `schema_version`, explicit operator/model/network boundary fields, profile state/binary/run support, install behavior, network boundary, and runner-specific guard details for runnable TypeScript/Pyright/gopls/clangd profiles. Extra readiness operands and unknown readiness options reject with usage; slash completion now covers `--json` for list/status and inspect/show.
