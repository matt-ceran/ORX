...
- Dependency-free local code maps, symbol indexes, and reference indexes are implemented through `orx code map`, `orx map`, `orx code-map`, `orx code symbols`, `orx symbols`, `orx code refs`, `orx refs`, `/map`, `/code map`, `/code symbols`, `/symbols`, `/code refs`, and `/refs`; output is local-only/no-key and includes bounded language, key-file, entrypoint, JavaScript/TypeScript import/export, exported-symbol, and code-reference summaries.
