...
- The gate runs `git diff --check`, `npm run typecheck`, full `npm test`, `npm run verify:global-install`, and built CLI smokes for `doctor --json`, `guide`, `code calls`, `plugins review`, `plugins review --json`, and `mcp presets`.
