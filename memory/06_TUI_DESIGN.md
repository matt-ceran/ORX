...
- TTY chat uses a compact bottom status notch and framed `╰─ orx ›` composer instead of the older long header/footer. A TTY screen controller tracks the visible composer line count, clears submitted composer blocks, and redraws idle/activity composers in place. Non-TTY and `NO_COLOR=1` keep the plain `orx>` line-oriented fallback.
...
- The TTY model badge uses compact route labels for OpenRouter routing shortcuts: `openrouter/auto` renders as `route auto`, and `openrouter/fusion` renders as `route fusion`. On wider TTY layouts exact `provider/model` ids render as separate `provider` and `model` badges; narrower layouts keep a single compact model badge. Full ids remain visible in plain status and request/config surfaces.
- The TTY bottom HUD keeps model/mode prominent and cwd/session/permissions secondary. Context and cost meters live in the bottom area; context remains approximate local bytes, cost remains observed OpenRouter metadata coverage/known cost, and assistant/tool activity frames add a subtle pulse marker to those meters without faking usage data.
...
- TTY chat shows a subtle `work <spinner> assistant` activity state while waiting for assistant output and `work <spinner> tool <name>` while native tools run. The activity composer redraws in place and clears before assistant/tool scrollback is printed, avoiding repeated HUD blocks in scrollback.
- `renderTtyStatusProbe(width)` provides a small pure render probe for 80, 120, and narrow-width HUD checks. `scripts/probe-tty-render.mjs` also shows launch/idle, escaped redraw trace, and assistant-turn frames. Pixel-level PTY/xterm/browser screenshot verification is still a future visual QA harness, not part of the current dependency set.
...
Next TTY polish should focus only on optional future raw-mode editing or xterm/pixel visual QA if it can preserve the current script-safe fallback.
